...
$id = $_GET['id'];
$data = read("SELECT slip_gajih.id,slip_gajih.tanggal,pegawai.nama_lengkap,slip_gajih.total_gajih,slip_gajih.potongan, pegawai.tunjangan,pegawai.gajih_pokok,pegawai.nama_lengkap FROM `slip_gajih` INNER JOIN `pegawai` on slip_gajih.pegawai_id =pegawai.id WHERE slip_gajih.id	='$id'")[0];
?>
			...
			<table class="table" id="tabel_slip">
				<thead>
					...
					<tr>
						...
						<th scope="col"><?= $data[3] + $data[6]- $data[5] - $data[4] ?></th>
					</tr>
				</thead>
			</table>
